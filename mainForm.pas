...
{$R *.dfm}
  ...
  procedure TMainForm.TestPingClick(Sender: TObject);
    begin
      { with TPingResult.Create(self) do
        try
        ShowModal;
        finally
        Free;
        end; }
      // PingResultForm:=TPingResult.Create(self);
      PingResultForm := TPingResult.Create(Application);
      PingResultForm.setSSConf(LoginResult);
      PingResultForm.ShowModal;
      { if not Assigned(PingResultForm) then
        Application.CreateForm(TPingResult, PingResultForm);
        PingResultForm.Parent := MainForm_var;
        PingResultForm.ShowModal; }
    ...
    begin
      ...
      if (email = '') and (password = '') then
        ...
        begin
          if formhash <> '' then
            begin
              MessageBox(self.Handle, PChar('即将登陆,登陆时间长度可能有所不一'),
                PChar('注意'), MB_OK);
